...
def test_hamming_distance_rule():
    ...
    G = Graph(K, connections, agents)
    assert G.hamming_distance_rule(agents[0]) == [agents[0], agents[1]]

    K = BeliefBase([Prop.P], [])
    agents: List[Interpretation] = [[Z2(0)], [Z2(1)]]
    connections: List[Connection] = [(agents[0], agents[0]), (agents[0], agents[1]), (agents[1], agents[1])]
    G = Graph(K, connections, agents)
    assert G.hamming_distance_rule(agents[1]) == [agents[1]]
    ...
    G = Graph(K, connections, agents)
    G.complete_graph()
    assert G.hamming_distance_rule([Z2(1), Z2(0), Z2(0)]) ==  [
        [Z2(0), Z2(0), Z2(1)],
        [Z2(1), Z2(0), Z2(0)],
        [Z2(1), Z2(1), Z2(1)]
    ]
